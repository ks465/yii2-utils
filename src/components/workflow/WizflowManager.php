 ...
 */
class WizflowManager extends \raoul2000\wizflow\WizflowManager
{
    /**
     * @var string namespace of the parent directory containing wiazards
     */
    public $wizardNamespace ='';
	/**
...
//			['class'=>$status->getMetadata('model')],
			$attributes
		);
...
//		$config = ['class'=> $status->getMetadata('model')];
		$config = $this->getModel($status);
//var_dump($config);exit;
     ...
     * @return array
     */
    private function getModel($status) {
//        var_dump($status->getMetadata('model'));exit;
	    return [
	        'class'=>/*$this->wizardNamespace . '\\wizflow\\models\\' .*/ $status->getMetadata('model'),
            'scenario'=>$status->getMetadata('model'),
            ];
    }
}
